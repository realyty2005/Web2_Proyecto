...
const listaProductos = document.getElementById("listas-cartas"); // Corregido el nombre del ID
...
function mostrarCarrito() {
    ...
    const total = articulosCarrito.reduce((total, producto) => {
        // Agrega un log para verificar el precio del producto actual
        console.log(`Precio: ${producto.precio}`);
        return total + (parseFloat(producto.precio.replace('$', '')) * producto.cantidad);
    }, 0);

    // Agrega un log para verificar el total antes de asignarlo
    console.log(`Total antes de redondeo: ${total}`);
